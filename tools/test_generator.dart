...
void main() async {
  ...
  final modules = await modulesDir.list().where((entity) => entity is Directory).toList();
  ...
  for (final module in modules) {
    ...
    if (await modelsDir.exists()) {
      final models = await modelsDir.list().where((f) => f.path.endsWith('.dart')).toList();
      print('    └─ Modèles: ${models.length}');
    }
    
    if (await servicesDir.exists()) {
      final services = await servicesDir.list().where((f) => f.path.endsWith('.dart')).toList();

...
class RouteGuard {
  ...
  static bool canAccess(BuildContext context, String route) {
    ...
    if (AppRoutes.requiresAuth(route)) {
      ...
      _logger.debug(
        'Route protégée - Utilisateur authentifié: $isAuthenticated',
      );
  ...
  static String getRedirectRoute(BuildContext context, String intendedRoute) {
    ...
    if (AppRoutes.requiresAuth(intendedRoute) && !authProvider.isLoggedIn) {
    ...
    if (authProvider.isLoggedIn && intendedRoute.startsWith('/auth/')) {
  ...
  static Future<void> saveIntendedRoute(String route) async {
    if (AppRoutes.requiresAuth(route)) {
      _logger.debug('Sauvegarde de la route de destination: $route');
      await CacheService().set('intended_route', route);
    }
  }

  /// Version synchrone pour GoRouter (utilise un stockage temporaire)
  static void saveIntendedRouteSync(String route) {
    if (AppRoutes.requiresAuth(route)) {
      _logger.debug('Sauvegarde synchrone de la route de destination: $route');
      // Utiliser un stockage temporaire en mémoire pour GoRouter
      _tempIntendedRoute = route;
      // Sauvegarder de manière asynchrone en arrière-plan
      _saveIntendedRouteAsync(route);
    }
  }

  /// Variable temporaire pour le stockage synchrone
  static String? _tempIntendedRoute;

  /// Sauvegarde asynchrone en arrière-plan
  static void _saveIntendedRouteAsync(String route) {
  ...
  static Future<String?> getAndClearIntendedRoute() async {
    // D'abord vérifier le stockage temporaire
    if (_tempIntendedRoute != null) {
      final route = _tempIntendedRoute;
      _tempIntendedRoute = null;
      _logger.debug(
        'Route de destination récupérée du stockage temporaire: $route',
      );
      return route;
    }

    // Puis vérifier le cache persistant
    final intendedRoute = await CacheService().get<String>('intended_route');
    if (intendedRoute != null) {
      await CacheService().delete('intended_route');
      _logger.debug(
        'Route de destination récupérée et effacée: $intendedRoute',
      );
    }
    return intendedRoute;
  }
  ...
  static bool hasSpecialPermission(BuildContext context, String route) {
    ...
    if (route.startsWith('/moderate/')) {
      return authProvider.isLoggedIn &&
          (authProvider.userData?['role'] == 'admin' ||
              authProvider.userData?['role'] == 'moderator');
    }
  ...
  static Future<bool> navigateTo(BuildContext context, String route) async {
    ...
    switch (guardResult.action) {
      ...
      case RouteGuardAction.redirect:
        ...
        await saveIntendedRoute(guardResult.originalRoute!);

        // Vérifier que le contexte est toujours valide après l'opération async
        if (context.mounted) {
  ...
  static Future<bool> navigateAndReplace(

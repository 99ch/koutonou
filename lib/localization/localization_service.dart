// Service de localisation simplifié pour l'application Koutonou.
// Fournit un accès centralisé aux traductions avec support pour le français et l'anglais.
// Version simplifiée sans cache pour la production.
...
class LocalizationService {
  ...
  /// Sauvegarde la locale (version simplifiée)
  Future<void> _saveLocale(Locale locale) async {
    try {
      // Version simplifiée : juste garder en mémoire
      _currentLocale = locale;
      _logger.info('Locale sauvegardée: ${locale.languageCode}');
    } catch (e) {
      _logger.error('Erreur lors de la sauvegarde de la locale: $e');
    }
  }

  /// Récupère la locale sauvegardée (version simplifiée)
  Future<Locale?> _getSavedLocale() async {
    try {
      // Version simplifiée : utiliser la locale par défaut

// Service de base amélioré pour l'API PrestaShop - Phase 3
// Intègre la configuration, gestion d'erreurs, cache et retry automatique
...
abstract class PrestaShopBaseService<T> {
  ...
  Future<List<T>> getAll({
    ...
    return await PrestaShopErrorHandler.handleWithRetry(() async {
  ...
  Future<T> create(T item) async {
    return await PrestaShopErrorHandler.handleWithRetry(() async {
      ...
      final requestBody = {
        resourceName.substring(0, resourceName.length - 1): itemJson, // Remove 's'
      };
  ...
  Future<T> update(int id, T item) async {
    return await PrestaShopErrorHandler.handleWithRetry(() async {
      ...
      onError: (error) => _logger.error('Erreur update $resourceName $id: $error'),
      onRetry: (attempt, error) => _logger.warning(
  ...
  Future<bool> delete(int id) async {
    return await PrestaShopErrorHandler.handleWithRetry(() async {
      ...
      onError: (error) => _logger.error('Erreur delete $resourceName $id: $error'),
      onRetry: (attempt, error) => _logger.warning(
  ...
  List<T> _parseListResponse(Map<String, dynamic> response) {
    try {
      ...
      _logger.warning('Structure de réponse inattendue pour $resourceName: ${response.keys}');
      return [];
  ...
  T? _parseSingleResponse(Map<String, dynamic> response) {
    try {
      final resourceKey = resourceName.substring(0, resourceName.length - 1); // Remove 's'
